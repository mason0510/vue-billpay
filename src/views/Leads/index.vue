<template>
	<div class="page">
		...
		<div class="leads_infos flex">
			<div class="leads_info ta_c">
				<div class="leads_info_num flex_c">{{ customerData.dayTotal }}</div>
				<div class="leads_info_txt">今日客户引流</div>
			</div>
			<div class="leads_info ta_c">
				<div class="leads_info_num flex_c">{{ customerData.total }}</div>
				<div class="leads_info_txt">累计客户引流</div>
			</div>
			<div class="leads_info_btn flex flex_align_c"
		 		@click="jump" data-url="/Leads/set"
				>
				<img src="/img/icon_notice@2x.png" alt="" class="leads_info_btn_icon bxz no_shrink">
				<div class="leads_info_btn_txt">提醒设置</div>
			</div>
		</div>
		...
		<div class="leads_list"  @click='isVip==1?"":jump($event)' data-url='/Leads/open' >
			...
			>
				...
				<div class="" v-for="(item,index) in customerList">

					<!-- v-for="(item,index) in 5" :key='index'  -->
					<div class="leads_li"
						>

						<div class="leads_li_time">{{ item.formatCreateTimeText }}</div>
						...
						<div class="leads_li_info_box flex lh1" v-if='isVip==1'>
							...
							<div class="leads_li_info flex_grow">
								...
								<div class="leads_li_tel" v-if="item.mobile">{{ item.mobile }}</div>
								<div class="leads_li_tel" v-else>用户未录入手机号码</div>
						...
						<div class="leads_li_article flex flex_align_c"
					 		@click.stop="jump" data-url="/Leads/info" :data-id='item.articleId' :data-article-type="item.type"
							>
							<img v-if="item.type==2" src="/img/icon_video.jpg" alt="" class="leads_li_article_icon no_shrink">
							<img v-else src="/img/icon_text@2x.png" alt="" class="leads_li_article_icon no_shrink">
						...
						<div class="leads_li_tips flex flex_align_c">
							<div class="leads_li_reads flex">
								<div>阅读 </div>
								<div class="leads_li_read_red">{{ item.retentionText }}</div>
							</div>
		...
		<div class="global_btn flex_c op_0" >108条客户线索，点击查看</div>
		<!--  v-if='types!="member"' @click='types=="member"?"":jump($event)' data-url='/Leads/open' -->
		<div class="global_btn flex_c fixed_btn" @click='isVip==1?"":jump($event)' data-url='/Leads/open'  v-if="isVip==0" >{{ customerData.total }}条客户线索，点击查看</div>
	...
	export default {
		...
		methods: {
            ...
            getCustomerCueList() {
                ...
                let self = this;
                axios.post(uri, {beforeUpdatedAt: self.beforeUpdatedAt}).then(function (response) {
                    let data = response.data.data;
