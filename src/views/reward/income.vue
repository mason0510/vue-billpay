    ...
    <div class="page">
        ...
        <div class="profit_page bg_white">
            ...
            <div>
                ...
                <div class="profit_title flex flex_ac">{{ dateTime }} 收益来源</div>
                        ...
                        <div class="" v-for="(item,index) in list" :key="index" >
                            ...
                            <div class="profit_area flex flex_ac">
                                ...
                                <div class="profit_area_info flex_grow">
                                    ...
                                    <div class="profit_area_tip txt_over_ell">注册时间：{{ item.registerTime | formatTimer('yyyy-MM-dd hh:mm:ss') }}</div>
                                    <div class="profit_area_tip txt_over_ell">开通时间：{{ item.vipTime | formatTimer('yyyy-MM-dd hh:mm:ss') }}</div>
                                </div>
                                <div class="profit_area_nums ">
                                    <div class="profit_area_num">+{{ item.price }}</div>
                                    <div class="profit_area_btn flex_c float_r">{{ item.serviceName }}</div>
                                </div>
                            </div>
    ...
    export default {
        ...
        methods:{
            ...
            getIncomeList() {
                let self = this;
                let uri = "income/list";
                let param = {beforeId:self.beforeId,dateTime:self.dateTime};
                axios.post(uri,param).then(function (response) {
                    let data = response.data.data;
                    self.list = [...self.list,...data.incomes];
                    self.loading=false;
                    self.beforeId = data.incomeBefore;
                    data.incomes.length==0&&(self.finished=true);
                });
            },
        },
        filters:{
