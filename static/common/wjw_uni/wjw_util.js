...
function token_false(res){
    ...
    */
    ...
    uni.showToast({
        // title: uni.getStorageSync('token')?'登录失效，请重新登录':'未登录，请先登录',
        title: res.data.msg,
        icon: 'none',
    });
    setTimeout(res=>{
        ...
        if(window&&window.webkit){
            window&&window.webkit&&window.webkit.messageHandlers.loginOut.postMessage('');
            return
        }
        uni.reLaunch({
            ...
            complete(res){
                ...
                if(judge_url_pre(getUrl(url))){
                    ...
                    /* 异地登录 token
                        ...
                        prompt('token'+uni.getStorageSync('token'), JSON.stringify(res));
                        {
                            "statusCode":200,
                            "data":{
    							// token失效
